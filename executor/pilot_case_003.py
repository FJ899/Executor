...
from executor.pilot_case_002 import PilotCase002DockerSandboxBackend
...
class PilotCase003WorkerError(PilotCase001Error):
    pass
...
def apply_case_003_worker(
    ...
    *,
    ...
    try:
        ...
        raise PilotCase003WorkerError(f"cannot read worker target: {exc}") from exc
    if source.count(_BROKEN_TO_PAYLOAD) != 1:
        raise PilotCase003WorkerError(
            "pinned CASE-003 defect was not found exactly once"
        )
    try:
        path.write_text(
            source.replace(_BROKEN_TO_PAYLOAD, _FIXED_TO_PAYLOAD),
            encoding="utf-8",
        )
    except OSError as exc:
        raise PilotCase003WorkerError(f"cannot write worker target: {exc}") from exc
...
def verify_case_003_output_checkout(
    ...
    *,
    ...
    if _git_stdout(root, "status", "--porcelain", "--untracked-files=all"):
        raise PilotCase001PolicyError("CASE-003 output worktree is not clean")
    ...
    if len(commit_line) != 2 or commit_line[1] != contract.input_commit:
        raise PilotCase001PolicyError(
            "CASE-003 output must be one commit directly on pinned input"
        )
    changed = _changed_paths(root, contract.input_commit, output_commit)
    if changed != (contract.allowed_path,):
        raise PilotCase001PolicyError(
            f"CASE-003 changed paths are not allowed: {list(changed)}"
        )
    return root
...
def case_003_sandbox_spec(image: str) -> SandboxSpec:
    return replace(
        ...
        labels={"creative-os-executor-pilot": "case-003"},
    )


class PilotCase003DockerSandboxBackend(PilotCase002DockerSandboxBackend):
    """Narrow external-project exception for the pinned CASE-003 pilot only."""
    ...
    def authorize(self, context: SandboxExecutionContext) -> Path:
        ...
        if policy.external_projects:
            raise SandboxExecutionError(
                "CASE-003 pilot requires global external project execution to remain disabled"
            )
        if policy.auto_merge:
            raise SandboxExecutionError("CASE-003 pilot forbids auto merge")
        if policy.default_network or policy.default_secrets:
            raise SandboxExecutionError(
                "CASE-003 pilot requires network=false and no default secrets"
            )
        ...
        try:
            root = verify_case_003_output_checkout(
            ...
            raise SandboxExecutionError(
                f"Unverified CASE-003 output: {exc}"
            ) from exc
        ...
        try:
            ...
            raise SandboxExecutionError(
                f"CASE-003 source cannot be resolved: {exc}"
            ) from exc
        if source != root:
            raise SandboxExecutionError(
                "CASE-003 sandbox must mount the verified output repository root"
            )
        try:
            verify_source_tree(root, commit=context.commit, source_dir=root)
        except RepositorySnapshotError as exc:
            raise SandboxExecutionError(
                f"CASE-003 source does not match the output commit: {exc}"
            ) from exc
        return root
...
def execute_case_003(
    *,
    ...
    try:
        ...
        apply_case_003_worker(worktree, contract=contract)
        pending = tuple(
            line
        ...
        _git(
            ...
            "commit",
            "-m",
            "Fix CASE-003 canonical project ordering",
        )
