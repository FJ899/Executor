...
It requires an `ExecutionPolicySnapshot` loaded from `EXECUTOR_POLICY.yaml` in a verified `litrgratis-pixel/Executor` checkout. The file must match the blob at the exact checked-out commit. The snapshot records the policy SHA-256 and cannot be constructed through its public dataclass fields.
...
A create failure, ownership mismatch, daemon failure or unverified absence is fail-closed. The result records the execution ID and policy digest used for the run.
