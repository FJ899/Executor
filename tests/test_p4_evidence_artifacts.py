...
class P4EvidenceArtifactTests(unittest.TestCase):
    ...
    def test_request_and_external_candidate_artifacts_are_exactly_bound(self):
        for name, issue_number in self.CASES:
            with self.subTest(name=name):
                request = json.loads(
                    (
                        ROOT / f"evidence/p4/requests/{name}-request.json"
                    ).read_text()
                )
                candidate = json.loads(
                    (
                        ROOT / f"evidence/p4/candidates/{name}-solution-candidate.json"
                    ).read_text()
                )
                issue_url = (
                ...
                issue = {
                    "url": issue_url,
                    "repository_url": (
                        f"https://api.github.com/repos/{PROFILE.intake_repository}"
                    ),
                    "number": issue_number,
                ...
                for mutation in candidate["mutations"]:
                    self.assertEqual(
                        hashlib.sha256(
                            mutation["replacement_text"].encode("utf-8")
                        ).hexdigest(),
