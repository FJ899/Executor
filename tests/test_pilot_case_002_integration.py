...
@unittest.skipUnless(
    os.environ.get("RUN_PILOT_CASE_002") == "1",
    "real CASE-002 pilot is opt-in",
)
class PilotCase002IntegrationTest(unittest.TestCase):
    def test_real_pinned_target_is_repaired_and_tested_in_docker(self):
        target = Path(os.environ["PILOT_TARGET_ROOT_CASE_002"]).resolve(strict=True)
        ...
        backend = PilotCase002DockerSandboxBackend(policy_snapshot=snapshot)
        ...
        with tempfile.TemporaryDirectory() as temp:
            report = execute_case_002(
                ...
                sandbox_spec=case_002_sandbox_spec(image),
            )
            try:
                self.assertEqual(
                    report["status"],
